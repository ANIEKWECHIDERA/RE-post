...
import { acceptedMediaMimeTypes, maxMediaFileSize, mediaMetadataSchema } from "@/schemas/media";
...
export async function createComposerPostAction(
  ...
  if (!supabase || !user) {
    return {
      ok: false,
      message: "Sign in with a configured Supabase project before creating posts.",
    };
  }
  ...
  const scheduledAt =
    typeof rawScheduledAt === "string" && rawScheduledAt ? new Date(rawScheduledAt).toISOString() : undefined;
  ...
  for (const file of files) {
    if (!acceptedMediaMimeTypes.includes(file.type as (typeof acceptedMediaMimeTypes)[number])) {
      return {
        ok: false,
  ...
  const targetStatus: PostTargetStatus = parsed.data.scheduleMode === "scheduled" ? "queued" : "pending";
  const postStatus = parsed.data.scheduleMode === "scheduled" ? "scheduled" : "queued";
  ...
  if (postError) {
    return {
      ok: false,
      message: "Post could not be saved. Confirm the Phase 2 migrations are applied.",
    };
  }
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    if (uploadError) {
      return {
        ok: false,
        message: "Media upload failed. Confirm the private post-media bucket exists.",
      };
    }
    ...
    const { error: postMediaError } = await supabase.from("post_media_assets").insert({
      post_id: postId,
  ...
  const { error: targetError } = await supabase.from("post_platform_targets").insert(targetRows);
  ...
  await supabase.from("activity_events").insert({
    user_id: user.id,
    post_id: postId,
    type: parsed.data.scheduleMode === "scheduled" ? "post_scheduled" : "publish_queued",
    title: parsed.data.scheduleMode === "scheduled" ? "Post scheduled" : "Post queued",
    message:
  ...
  return {
    ...
    message:
      parsed.data.scheduleMode === "scheduled"
        ...
        : "Post queued. The publishing engine will pick it up in Phase 7.",
    postId,
  };
}
