...
import { connectionActionSchema, connectionIdSchema } from "@/schemas/connection";
...
export async function prepareConnectionAction(_: ConnectionActionState, formData: FormData) {
  ...
  if (error) {
    return {
      ok: false,
      message: "OAuth state could not be prepared. Confirm Phase 6 migrations are applied.",
    };
  }
...
export async function revokeConnectionAction(_: ConnectionActionState, formData: FormData) {
  const user = await getCurrentUser();
  const supabase = await createSupabaseServerClient();
