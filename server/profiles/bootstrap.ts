...
export async function ensureProfileBootstrap(user: User): Promise<ProfileBootstrapResult> {
  ...
  if (profileError) {
    return {
      ok: false,
      message: "Profile bootstrap failed. Confirm the Phase 2 migration has been applied.",
    };
  }
  ...
  if (streakError) {
    return {
      ok: false,
      message: "Streak bootstrap failed. Confirm the Phase 2 migration has been applied.",
    };
  }
